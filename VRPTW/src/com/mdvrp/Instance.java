...
public class Instance {
	...
	public void populateFromHombergFile(String filename) {
		try {
			...
			{					
				...
				// these data are used for DrawPanel
			...
			depot.setNumber(customersNr);// the depot contains the number of clients it manages
			
			if(parameters.getTabuTenure() == -1) //se non ho preciso sulla riga di comando la tabuTenure
				parameters.setTabuTenure((int)(Math.sqrt(getCustomersNr())));
			
			calculateDistances();
			assignCustomersToDepots();			
			calculateAngles();
			sortAssignedCustomers(); //useless when we have only one deposito
		...
		for (int i = 0; i < customersNr; ++i){
			...
			for (int j = customersNr; j < customersNr + depotsNr; ++j)
				if (min > distances[i][j]) {
					min = distances[i][j];
					depotToAssign = j - customersNr; //iteration 1 3-3=0--> depot 0
				}
			customers.get(i).setAssignedDepot(depots.get(depotToAssign));//I set the dpot for the customer
			depots.get(depotToAssign).addAssignedCustomer(customers.get(i));//add the customer to the list of customer contained by the depot
		}
	...
	public void initializeRoutes() {//routes représente les différents chemins de chaque vehicules pour chaque dépot
		                            //il contient la liste des customers présent sur le chemin.
...
distances[j][i] = distances[i][j];
	...
	public void setListCustomer(List<Customer> l){
		this.customers=(ArrayList<Customer>) l;
	}
	
	public double restituisciDistance (int index1, int index2)
	{
